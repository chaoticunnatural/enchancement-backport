...
public interface MultiplyMovementSpeedEvent {
	...
	static float getJumpStrength(LivingEntity entity, float multiplier) {
		double jumpRatio = entity.getJumpVelocity() / entity.getJumpBoostVelocityModifier();
		double speedRatio = entity.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED) / entity.getAttributeBaseValue(EntityAttributes.GENERIC_MOVEMENT_SPEED);
		if (entity.isSprinting()) {
			speedRatio /= 1.3;
		}
		return (float) (MathHelper.clamp(jumpRatio * Math.min(1, speedRatio), 2 / 3F, 1.2F) * entity.getJumpBoostVelocityModifier() * multiplier);
	}
	...
	static float getMovementMultiplier(LivingEntity entity) {
		...
		return (float) MathHelper.clamp(entity.getAttributeValue(EntityAttributes.GENERIC_MOVEMENT_SPEED) * multiplier / entity.getAttributeBaseValue(EntityAttributes.GENERIC_MOVEMENT_SPEED), 2 / 3F, 1.2F);
	}
}
